...
/*(function(window){*/
  ...
  function selector2id(selector){
    ...
    var sex2 = selector.match(/,[mw]|([olx]s)|(d(&[ol]))$/)?0:1;
  ...
  var $btns = DOMUtil.getElementsByClassName('btn-default');
  var $buttons = DOMUtil.getElementsByClassName('jisuan');
function  relate(){
...
/*计算*/
 /* $buttons[0].onclick = */function  btnCal() {
    var value = $textarea[0].value.trim();
    var result = relationship(value.replace(new RegExp(" ", "g"),""));
    $textarea[1].value = '';
    if (result.length) {
      for (var i = 0; i < result.length; i++) {
  ...
  /*$buttons[1].onclick= */ function  btnBack(){
    var value = $textarea[0].value.trim();
  ...
  /*$buttons[2].onclick = */function  btnClear(){
